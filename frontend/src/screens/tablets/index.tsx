...
export const TabletsComponent: React.FC = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [tabletData, setTabletData] = useState<IProduct[]>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          "https://api.mercadolibre.com/sites/MLB/search?q=tablets"
        );
        setTabletData(response.data.results);
      } catch (error) {
